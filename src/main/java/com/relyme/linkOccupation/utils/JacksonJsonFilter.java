...
public class JacksonJsonFilter extends FilterProvider{
    ...
    Map<Class<?>, Set<String>> notincludeMap = new HashMap<>();

    public void include(Class<?> type, String[] fields) {
        addToMap(includeMap, type, fields);
    }

    public void notinclude(Class<?> type, String[] fields) {
        addToMap(notincludeMap, type, fields);
    }
    ...
    public boolean apply(Class<?> type, String name) {
        ...
        Set<String> noitincludeFields = notincludeMap.get(type);
        if (includeFields != null && includeFields.contains(name)) {
            return true;
        } else if (filterFields != null && !filterFields.contains(name)) {
            return true;
        }else if(noitincludeFields != null && noitincludeFields.contains(name)){
            return false;
