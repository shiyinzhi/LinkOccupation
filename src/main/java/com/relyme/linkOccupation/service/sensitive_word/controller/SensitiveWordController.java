package com.relyme.linkOccupation.service.sensitive_word.controller;
...
import com.relyme.linkOccupation.service.sensitive_word.dto.SensitiveWordDelDto;
import com.relyme.linkOccupation.service.sensitive_word.dto.SensitiveWordQueryDto;
import com.relyme.linkOccupation.service.sensitive_word.dto.SensitiveWordUpdateDto;
...
public class SensitiveWordController {
    ...
    @JSON(type = SensitiveWord.class)
    @RequestMapping(value="/findByConditionAPI",method = RequestMethod.POST,consumes = MediaType.APPLICATION_JSON_VALUE)
    public Object findByConditionAPI(@Validated @RequestBody SensitiveWordQueryDto queryEntity, HttpServletRequest request) {
        try{
            ...
            //查询默认当天的费用记录
            Specification<SensitiveWord> specification=new Specification<SensitiveWord>() {
                private static final long serialVersionUID = 1L;

                @Override
                public Predicate toPredicate(Root<SensitiveWord> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
                    List<Predicate> predicates = new ArrayList<>();
                    List<Predicate> predicates_or = new ArrayList<>();
                    Predicate condition_tData = null;
                    if(StringUtils.isNotEmpty(queryEntity.getSearStr())){
                        predicates_or.add(criteriaBuilder.like(root.get("content"), "%"+queryEntity.getSearStr()+"%"));
                    }
            ...
            Page<SensitiveWord> sensitiveWordPage = sensitiveWordDao.findAll(specification,pageable);
            List<SensitiveWord> content = sensitiveWordPage.getContent();
            content.forEach(sensitiveWord -> {
                UserAccount byUuid = userAccountDao.findByUuid(sensitiveWord.getUserAccountUuid());
                if(byUuid != null){
                    sensitiveWord.setCreaterName(byUuid.getName());
                }
            });
     ...
     */
    @ApiOperation("删除")
    @JSON(type = SensitiveWord.class  , include="uuid")
    @RequestMapping(value="/delByUuid",method = RequestMethod.POST,produces={"application/json;charset=UTF-8","text/html;charset=UTF-8"})
    public Object delByUuid(@Validated @RequestBody SensitiveWordDelDto entity, HttpServletRequest request) {
        try{
            ...
            SensitiveWord byUuid = sensitiveWordDao.findByUuid(entity.getUuid());
            if(byUuid == null){
                throw new Exception("敏感词信息异常！");
            }

            byUuid.setActive(2);
            sensitiveWordDao.save(byUuid);
    ...
    public Object activeSet(@Validated @RequestBody SensitiveWordDelDto entity,HttpServletRequest request) {
        try{
    ...
    public Object update(@Validated @RequestBody SensitiveWordUpdateDto entity, HttpServletRequest request) {
        try{
    ...
    private void updateSensitiveWord(){
        ...
        List<SensitiveWord> byActive = sensitiveWordDao.findByActive(1);

        List<String> sensitiveWords = new ArrayList<>();
        for (SensitiveWord sensitiveWord : byActive) {
            sensitiveWords.add(sensitiveWord.getContent());
        }

        loadSensitiveWord.setSensitiveWords(sensitiveWords);
    }
