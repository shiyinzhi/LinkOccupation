...
//                    predicates.add(condition_tData);
    ...
    public Object updatePaymentList(@Validated @RequestBody SocialSecurityUuidListDto queryEntity, HttpServletRequest request) {
        try{
            ...
            List<SocialSecurity> byUuidIn = socialSecurityDao.findByUuidIn(queryEntity.getUuids());
            if(byUuidIn == null || byUuidIn.size() == 0){
                throw new Exception("代缴社保信息异常！");
            }

            List<SocialSecurity> hasUpdates = new ArrayList<>();
            for (SocialSecurity socialSecurity : byUuidIn) {
                socialSecurity.setIsPayment(1);
                hasUpdates.add(socialSecurity);
            }

            socialSecurityDao.save(hasUpdates);

            for (SocialSecurity hasUpdate : hasUpdates) {
